...
import {Card, Stack, Link, Container, Typography, Grid, SvgIcon, Collapse} from '@material-ui/core';
...
export default function Login(props) {
  ...
  const handleClick = async (data) => {
    ...
    if (!signUpForm) {
      fetch(`${REACT_APP_BACKEND_URL}/users?publicAddress=${publicAddress}`)
        ...
        .then((users) => {
          if (users.length) {
            handleSignMessage(users[0]).then(handleAuthenticate).catch((err) => {
              window.alert(err);
              setLoading(false)
            })
          } else {
      ...
      if (data) {
        ...
        fetch(`${REACT_APP_BACKEND_URL}/users`, {
          ...
          method: 'POST',
        }).then((response) => response.json()).then(handleSignMessage).then(handleAuthenticate).catch((err) => {
          window.alert(err);
          setLoading(false);
        });
      }
  ...
  const handleAuthenticate = (
    ...
    fetch(`${REACT_APP_BACKEND_URL}/auth`, {
      ...
      response.json().then(onLoggedIn).catch((err) => window.alert(err))
    })};
